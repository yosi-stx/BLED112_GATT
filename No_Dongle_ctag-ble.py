...
# This variant uses the standard Windows BLE stack through the "bleak" package.
# It does NOT require a Silicon Labs BLED112 USB BLE dongle.
#
# Install once on the PC:
#     py -m pip install bleak
#
# The PC Bluetooth adapter must support Bluetooth Low Energy (BLE), not only
# Bluetooth Classic. The C-TAG device must still expose the same GATT UUIDs.
...
class BleakEventLoopRunner:
    """Run one persistent asyncio loop for all Bleak operations.

    Bleak is asynchronous, while the original application was written around
    synchronous pygatt calls. This small bridge lets the rest of the program
    keep calling device.char_read(), device.char_write() and device.subscribe()
    with almost no other changes.
    """
    ...
    def run(self, coroutine, timeout=30):
        """Run a coroutine on the Bleak loop and wait for its result."""
...
class BleakDeviceCompatibility:
    """Synchronous compatibility wrapper over a connected BleakClient.

    Only the pygatt-style operations that are actually used by this program
    are implemented here. Existing packet parsing, GUI code and GUI command
    ...
    def char_read(self, uuid):
        return bytearray(self._runner.run(self._client.read_gatt_char(uuid)))

    def char_write(self, uuid, data, wait_for_response=True):
        # The original code requests a GATT write-with-response.
        # Keep that exact behavior by passing response=True.
        self._runner.run(
    ...
    def subscribe(self, uuid, callback):
        # Bleak notification callbacks receive (sender, bytearray). The
        # original callbacks already use the same two-argument shape.
        def notification_callback(sender, data):
            callback(sender, bytearray(data))

        self._runner.run(self._client.start_notify(uuid, notification_callback))

    def unsubscribe(self, uuid, *unused_args):
        # *unused_args preserves compatibility with the original call:
        # device.unsubscribe(MY_CHAR_UUID, False)
        self._runner.run(self._client.stop_notify(uuid))
...
async def connect_ble_device(device_reference):
    """Connect with the Windows BLE stack and return the live Bleak client."""
    client = BleakClient(device_reference, timeout=15.0)
...
def main_ble(global_vars):
    ...
    # Unlike pygatt.BGAPIBackend(), Bleak uses the normal Windows BLE stack
    # and the PC's built-in/standard BLE adapter. It needs one persistent
    # asyncio loop for scanning, connecting and GATT operations.
    bleak_runner = BleakEventLoopRunner()
    bleak_runner.start()

    global device

    try:
        # Scan for available BLE devices when selection by name/manual mode is
        # required. BleakScanner returns BLEDevice objects rather than pygatt
        # dictionaries, so address/name access is the only required change.
        if (do_scan):
            print("Scanning devices for %s seconds..." % str(SCAN_TIMEOUT))
            device_li = bleak_runner.run(
                BleakScanner.discover(timeout=SCAN_TIMEOUT),
                timeout=SCAN_TIMEOUT + 10
            )
            i = 1
            for d in device_li:
                print("%s. %s -- %s" % (str(i), str(d.address), str(d.name or "<unnamed>")))
                i += 1
        ...
        if (manual_mode):
            ...
            while (device_ind == None):
                ...
                try:
                    ...
                    print("Invalid input")
            # Pass the BLEDevice object from the scan directly to Bleak.
            # This avoids an unnecessary second discovery step.
            device_reference = device_li[device_ind]
            ...
            if (avail_name):
                ...
                error_address = False
                for d in device_li:
                    device_name = d.name or ""
                    if (verify_mode):
                        if (d.address == args.address[0] and device_name != args.name[0]):
                            error_name = True
                            break
                        if (d.address != args.address[0] and device_name == args.name[0]):
                            error_address = True
                            break
                    if (device_name == args.name[0] and ((not verify_mode) or (verify_mode and d.address == args.address[0]))):
                        # Keep the scanned object, not just its address.
                        device_reference = d
                        found = True
                        break
                if (not found):
                ...
                # Direct MAC-address connection remains supported.
                device_reference = args.address[0]

        # Connect using the standard Windows BLE adapter.
        print("\nConnecting to the selected device...")
        bleak_client = bleak_runner.run(connect_ble_device(device_reference))
        device = BleakDeviceCompatibility(bleak_client, bleak_runner)
        ...
        if device != None:
            try:
                ...
                print("Error while disconnecting BLE device:", error)
        bleak_runner.stop()
